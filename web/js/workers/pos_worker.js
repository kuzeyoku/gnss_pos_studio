...
self.onmessage = async function (event) {
  ...
  if (action === "PROCESS_SPP") {
    try {
      ...
      self.postMessage({
        type: "LOG",
        key: "rinex.workerGpsOrbitsLoaded",
        params: { count: satList.length }
      });
      self.postMessage({
        type: "LOG",
        key: "rinex.workerObsReading"
      });
        ...
        if (line.startsWith(">")) {
          ...
          if (currentEpochTime && Object.keys(currentPseudoranges).length > 0) {
            ...
            if (epochCounter % 500 === 0) {
              ...
              self.postMessage({
                type: "LOG",
                key: "rinex.workerSppEpoch",
                params: {
                  epoch: epochCounter,
                  solutions: solutions.length,
                  pdop: solutions.length > 0 ? solutions[solutions.length - 1].pdop.toFixed(2) : "-"
                }
              });
            }
          }
      ...
      if (solutions.length === 0) {
        throw new Error("ERR_GNSS_NO_SOLUTION");
      }
