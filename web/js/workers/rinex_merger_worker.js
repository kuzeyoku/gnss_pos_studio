  ...
  if (action === "MERGE_GROUP") {
    try {
      ...
      if (group.obsFiles && group.obsFiles.length > 0) {
        self.postMessage({
          type: "LOG",
          key: "rinex.workerObsFilesProcessing",
          params: { count: group.obsFiles.length }
        });
        ...
        self.postMessage({
          type: "LOG",
          key: "rinex.workerObsFileCreated",
          params: { filename: obsFilename, size: (mergedObsContent.length / 1048576).toFixed(2) }
        });
      ...
      if (group.navGpsFiles && group.navGpsFiles.length > 0) {
        self.postMessage({
          type: "LOG",
          key: "rinex.workerGpsNavProcessing",
          params: { count: group.navGpsFiles.length }
        });
        ...
        self.postMessage({
          type: "LOG",
          key: "rinex.workerGpsNavCreated",
          params: { filename: navGpsFilename, size: (mergedGpsNavContent.length / 1024).toFixed(1) }
        });
      ...
      if (group.navGloFiles && group.navGloFiles.length > 0) {
        self.postMessage({
          type: "LOG",
          key: "rinex.workerGloNavProcessing",
          params: { count: group.navGloFiles.length }
        });
        ...
        self.postMessage({
          type: "LOG",
          key: "rinex.workerGloNavCreated",
          params: { filename: navGloFilename, size: (mergedGloNavContent.length / 1024).toFixed(1) }
        });
      }
