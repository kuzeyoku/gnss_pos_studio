...
function initMergerDropzone() {
  ...
  async function v_5() {
    ...
    try {
      ...
      document.getElementById("statPpkBaselineDist").textContent = v_1_1.baselineKm > 0 ? v_1_1.baselineKm.toFixed(2) + " km" : t("rinex.lblNoAntennaXyz");
      document.getElementById("statPpkCommonSystems").textContent = v_1_1.commonConstellations.length > 0 ? v_1_1.commonConstellations.join(" + ") : t("rinex.lblNoCommonSystems");
      ...
      logMessage(t("rinex.logPpkAnalysisError", { err: v_1_1.message || v_1_1 }));
...
async function processUploadedFiles(arg1) {
  logMessage(t("rinex.logScanningHeadersCount", { count: arg1.length }));
  updateProgress(10, t("rinex.progressHeaderScan"));
  state.mergerGroups = {};
  const obj = {
  ...
  if (v_2.length === 0) {
    logMessage(t("rinex.logNoValidFilesDetected"));
    updateProgress(0, t("rinex.progressReady"));
    return;
  }
  ...
  if (v_3 && v_3.year) {
    ...
    if (domEl) {
      domEl.value = v_3.year >= 2026 ? "RINEX_304" : "RINEX_211";
      domEl.dispatchEvent(new Event("change"));
      populateDynamicFormats(domEl.value, v_3.year);
    }
  }
  if (obj.obsTypes.size > 0 && elements.selectPruneObsTypes) {
    elements.selectPruneObsTypes.innerHTML = "";
    Array.from(obj.obsTypes).sort().forEach(item => {
      const optionEl = document.createElement("option");
      optionEl.value = item;
      optionEl.textContent = item;
      optionEl.selected = true;
      elements.selectPruneObsTypes.appendChild(optionEl);
    });
  }
  logMessage(t("rinex.logStationsDetected", { count: v_2.length, sats: obj.constellations.size }));
  renderDynamicFilters(obj);
  renderGroupsTable();
  updateProgress(100, t("rinex.progressScanComplete"));
...
function renderDynamicFilters(arg1) {
  ...
  const obj_2 = {
    Phase: {
      label: t("rinex.chartCarrierPhase"),
      color: "var(--emerald-400)"
    },
    Code: {
      label: t("rinex.chartCodeDistance"),
      color: "var(--emerald-400)"
    },
    Doppler: {
      label: t("rinex.chartDoppler"),
      color: "var(--emerald-400)"
    },
    SNR: {
      label: t("rinex.chartSnrPower"),
      color: "var(--emerald-400)"
    }
  };
  if (domEl) {
...
window.mergeSingleGroup = async function (arg1) {
  ...
  logMessage(t("rinex.logProcessingGroupConfig", {
    group: arg1,
    systems: v_9 || t("common.all"),
    version: v_3,
    format: v_4,
    interval: v_5
  }));
  updateProgress(15, t("rinex.progressReadingContent"));
  const v_10 = arg1_1 => {
  ...
  v_14.onmessage = async arg1_1 => {
    ...
    if (v_2_1 === "LOG") {
      const msg = arg1_1.data.key ? t(arg1_1.data.key, arg1_1.data.params) : v_3_1;
      logMessage(msg);
    }
    if (v_2_1 === "PROGRESS") {
      updateProgress(v_4_1, t("rinex.progressProcessing"));
    }
    if (v_2_1 === "COMPLETE") {
      logMessage(t("rinex.logGroupSuccess", { name: arg1 }));
      ...
      if (v_3 === "RINEX_211" || v_3 === "RINEX_210") {
        if (v_4 === "OBS_YYO") {
          if (v_5_1.obs) {
            ...
            logMessage(t("rinex.logDownloadedObsYyo", { filename: v_5_1.obs.filename, version: str }));
          ...
          if (v_5_1.obs) {
            ...
            logMessage(t("rinex.logDownloadedObs", { filename: v_1_1, version: str }));
          ...
          if (v_5_1.navGps) {
            ...
            logMessage(t("rinex.logDownloadedNavGps", { filename: v_5_1.navGps.filename }));
          ...
          if (v_5_1.navGps) {
            ...
            logMessage(t("rinex.logDownloadedNav", { filename: v_1_1 }));
            showToast(t("rinex.toastFileDownloaded", { name: v_1_1 }), "success");
          } else {
            showToast(t("rinex.toastGpsNavNotFound"), "warning");
          }
          ...
          if (v_5_1.navGlo) {
            ...
            logMessage(t("rinex.logDownloadedNavGlo", { filename: v_5_1.navGlo.filename }));
          ...
          if (v_5_1.obs) {
            ...
            downloadTextFile(v_1_1, v_5_1.obs.content);
            logMessage(t("rinex.logDownloadedHatanaka", { filename: v_1_1, ext: String(v_2.year % 100).padStart(2, "0") + "d" }));
            showToast(t("rinex.toastFileDownloaded", { name: v_1_1 }), "success");
          }
        } else {
        ...
        if (v_4 === "OBS_YYO") {
          if (v_5_1.obs) {
            downloadTextFile(v_5_1.obs.filename, v_5_1.obs.content);
            logMessage(t("rinex.logDownloadedObs83", { filename: v_5_1.obs.filename, version: str }));
            showToast(t("rinex.toastFileDownloaded", { name: v_5_1.obs.filename }), "success");
          } else {
            showToast(t("rinex.toastObsNotFound"), "warning");
          }
          ...
          if (v_5_1.obs) {
            ...
            logMessage(t("rinex.logDownloadedObs", { filename: v_1_2, version: str }));
          ...
          if (v_5_1.obs) {
            downloadTextFile(v_1_1, v_5_1.obs.content);
            logMessage(t("rinex.logDownloadedMultiRnx", { filename: v_1_1, version: str }));
            showToast(t("rinex.toastFileDownloaded", { name: v_1_1 }), "success");
          }
        } else if (v_4 === "NAV_YYN") {
          if (v_5_1.navGps) {
          ...
          if (v_5_1.navGps) {
            ...
            logMessage(t("rinex.logDownloadedNav", { filename: v_1_2 }));
            showToast(t("rinex.toastFileDownloaded", { name: v_1_2 }), "success");
          } else {
            showToast(t("rinex.toastGpsNavNotFound"), "warning");
          }
          ...
          if (v_5_1.navGps) {
            ...
            downloadTextFile(v_1_2, v_5_1.navGps.content);
            logMessage(t("rinex.logDownloadedNavMix", { filename: v_1_2 }));
            showToast(t("rinex.toastFileDownloaded", { name: v_1_2 }), "success");
          }
        } else if (v_4 === "HATANAKA_YYD") {
          if (v_5_1.obs) {
            ...
            downloadTextFile(v_1_2, v_5_1.obs.content);
            logMessage(t("rinex.logDownloadedHatanaka", { filename: v_1_2, ext: String(v_2.year % 100).padStart(2, "0") + "d" }));
            showToast(t("rinex.toastFileDownloaded", { name: v_1_2 }), "success");
          }
          ...
          if (v_5_1.obs) {
            downloadTextFile(v_1_2, v_5_1.obs.content);
            logMessage(t("rinex.logDownloadedCrx", { filename: v_1_2 }));
            showToast(t("rinex.toastFileDownloaded", { name: v_1_2 }), "success");
          }
        } else {
          downloadGroupZip(arg1, v_5_1);
    ...
    if (v_2_1 === "ERROR") {
      logMessage(t("rinex.logError", { err: t(v_6_1) || v_6_1 }));
      v_14.terminate();
    }
  };
};
async function runSppOnMergedResults(arg1, arg2, arg3, arg4, arg5) {
  ...
  v_6.onmessage = arg1_1 => {
    ...
    if (v_2 === "LOG") {
      const msg = arg1_1.data.key ? t(arg1_1.data.key, arg1_1.data.params) : v_3;
      logMessage(msg);
    }
    if (v_2 === "PROGRESS") {
      updateProgress(v_4, t("rinex.progressSppCalculating"));
    }
    if (v_2 === "SPP_COMPLETE") {
      state.posSolutions = v_5;
      state.posText = v_6_1;
      state.csvText = v_7;
      logMessage(t("rinex.logSppSuccess", { count: v_8.totalEpochs }));
      logMessage(t("rinex.logSppCoords", {
        lat: v_8.avgLat.toFixed(8),
        lon: v_8.avgLon.toFixed(8),
        hgt: v_8.avgHeight.toFixed(3)
      }));
      if (arg5 === "NMEA_0183") {
        const v_1 = state.rinexEngine.generateNmeaLog(v_5);
        downloadTextFile(arg1 + ".nmea", v_1);
        logMessage(t("rinex.logDownloadedNmea", { name: arg1 }));
      } else if (arg5 === "CSV_TELEMETRY") {
        downloadTextFile(arg1 + ".csv", v_7);
        logMessage(t("rinex.logDownloadedCsv", { name: arg1 }));
      } else {
        downloadTextFile(arg1 + ".pos", v_6_1);
        logMessage(t("rinex.logDownloadedPos", { name: arg1 }));
      }
      v_6.terminate();
    }
    if (v_2 === "ERROR") {
      logMessage(t("rinex.logError", { err: t(v_9) || v_9 }));
      v_6.terminate();
    }
  };
}
...
function initPpkInspector() {
  ...
  const v_3 = () => {
    ...
    try {
      ...
      if (domEl_7) {
        domEl_7.textContent = t("rinex.lblCoverageVal", { pct: v_1_1.overlapPercent.toFixed(1) });
      }
      ...
      logMessage(t("rinex.logPpkDetails", {
        overlap: v_1_1.overlapPercent.toFixed(1),
        baseline: v_1_1.baselineKm.toFixed(2),
        duration: v_1_1.overlapDurationStr
      }));
      ...
      logMessage(t("rinex.logPpkError", { err: v_1_1.message || v_1_1 }));
...
function initRinexQualityInspector() {
  ...
  inputEl?.addEventListener("change", async arg1 => {
    ...
    try {
      ...
      document.getElementById("statQualityScore").textContent = "%" + v_2_1.qualityScore + " " + (v_2_1.qualityScore >= 80 ? t("rinex.qualityExcellent") : t("rinex.qualityGood"));
      drawRinexQualityChart(v_2_1.timeline);
      logMessage(t("rinex.logQualityScoreSummary", {
        name: v_2.name,
        epochs: v_2_1.totalEpochs,
        sats: v_2_1.avgSats,
        score: v_2_1.qualityScore
      }));
      showToast(t("rinex.toastAnalysisSuccess", { name: v_2.name, sats: v_2_1.avgSats }), "success");
    } catch (v_1) {
      logMessage(t("rinex.logQualityError", { err: v_1.message || v_1 }));
