...
class GnssFormatEngine {
  ...
  parseData(content, fileName = "") {
    ...
    if (text.startsWith("<?xml") || text.includes("<JOBFile") || text.includes("<FieldBook") || fileName.toLowerCase().endsWith(".jxl")) {
      this.detectedBrandCode = "BRAND_XML_GEODETIC";
      this.detectedBrand = (typeof window !== "undefined" && typeof window.t === "function") ? window.t("cadastre.brandXmlGeodetic") : "XML-Based Geodetic Data";
    ...
    if (text.includes("GPS,") || text.includes("GS,") || text.includes("BP,") || text.includes("EP,") || text.includes("SP,") || text.includes("JB,") || text.includes("--")) {
      this.detectedBrandCode = "BRAND_FIELD_STREAM";
      this.detectedBrand = (typeof window !== "undefined" && typeof window.t === "function") ? window.t("cadastre.brandFieldStream") : "Field Measurement Stream";
    ...
    if (firstLine.includes(",") || firstLine.includes(";") || firstLine.includes("\t")) {
      this.detectedBrandCode = "BRAND_DELIMITED_COORDS";
      this.detectedBrand = (typeof window !== "undefined" && typeof window.t === "function") ? window.t("cadastre.brandDelimitedCoords") : "Delimited Coordinate Stream";
      return this.parseCsv(text);
    }

    // 4. Serbest Sütunlu Sayı Dizisi
    this.detectedBrandCode = "BRAND_FREE_COORDS";
    this.detectedBrand = (typeof window !== "undefined" && typeof window.t === "function") ? window.t("cadastre.brandFreeCoords") : "Free Column Coordinate Stream";
  ...
  exportFormattedCoordinateList(formatType = "Pt,E,N,h") {
    ...
    for (let p of this.rawPoints) {
      ...
      const hVal = (this.isTg20Applied && p.orthoH !== null && p.orthoH !== undefined && !isNaN(p.orthoH))
        ? p.orthoH
        : p.h;
      const hStr = hVal !== null && hVal !== undefined && !isNaN(hVal) ? Number(hVal).toFixed(3) : "-";
  ...
  /**
   * Metin Bazlı TG-20 İndirgeme Raporu Veri Nesnesini (DTO) Hazırlar
   */
  getTg20ReductionReportData(projectName = "GNSS_RTK_TG20_Indirgeme") {
    const reportPoints = [];
    ...
    for (let pair of this.matchedPairs) {
      if (pair.isTg20Applied && pair.avgOrthoH) {
        reportPoints.push({
          name: pair.pointName,
          lat: pair.p1?.lat || pair.p1?.latDec || pair.p2?.lat || pair.p2?.latDec || null,
          lon: pair.p1?.lon || pair.p1?.lonDec || pair.p2?.lon || pair.p2?.lonDec || null,
          y: pair.avgE,
          x: pair.avgN,
          h: pair.avgH,
          N: pair.tg20N,
    ...
    for (let pt of this.unmatchedPoints) {
      if (pt.isTg20Applied && pt.orthoH !== undefined) {
        reportPoints.push({
          name: pt.pn,
          lat: pt.lat || pt.latDec || null,
          lon: pt.lon || pt.lonDec || null,
          y: pt.e != null ? pt.e.toFixed(3) : null,
          x: pt.n != null ? pt.n.toFixed(3) : null,
          h: pt.h != null ? pt.h.toFixed(3) : null,
          N: pt.tg20N,
          H: pt.orthoH != null ? pt.orthoH.toFixed(3) : null,
          typeCode: "SINGLE_OBS",
          isPair: false
        });
      }
  ...
  /**
   * Metin Bazlı TG-20 İndirgeme Raporu Üretir (Şablon katmanına delege eder)
   */
  exportTg20ReductionReport(projectName = "TG20_REDUCTION") {
    const data = this.getTg20ReductionReportData(projectName);
    if (typeof window !== "undefined" && window.GnssReportTemplates?.renderTg20TextReport) {
      return window.GnssReportTemplates.renderTg20TextReport(data);
    }
    return JSON.stringify(data);
  }

  /**
   * TG-20 Jeoit Raporu için saf hesaplama ve koordinat veri modelini üretir (Pure Data DTO)
   * UI veya HTML etiketleri barındırmaz, doğrudan render katmanına aktarılır.
   */
  getTg20ReportData(projectName = "TG20_REPORT", externalTg20 = null, externalGeodesy = null) {
    const reportRows = [];
    ...
    return {
      ...
      centralMeridian: this.centralMeridian,
      totalPoints: reportRows.length,
      matchedCount: this.matchedPairs.length,
      unmatchedCount: this.unmatchedPoints.length,
      avgUndulation: avgUndulation,
      rows: reportRows
    };
  }

  /**
   * Yazdırılabilir / PDF Kaydedilebilir A4 TG-20 Raporu Üretir
   * Şablon doldurma (hydration) işini doğrudan GnssReportTemplates yöneticisine devreder.
   */
  generatePrintableTg20Report(projectName = "TG20_REPORT", externalTg20 = null, externalGeodesy = null) {
    const reportData = this.getTg20ReportData(projectName, externalTg20, externalGeodesy);
    if (typeof window !== "undefined" && window.GnssReportTemplates?.renderTg20Report) {
      return window.GnssReportTemplates.renderTg20Report(reportData);
    }
    return "";
  }
...
`;
    ...
    <ExtendedData>
      <Data name="Point_Name"><value>${pair.pointName}</value></Data>
      <Data name="Diff_2D_cm"><value>${pair.ds2d}</value></Data>
      <Data name="Status_Dist"><value>${pair.isDistPassed ? "PASS" : "FAIL"}</value></Data>
      <Data name="Time_Diff_Min"><value>${pair.timeDiffMin}</value></Data>
      <Data name="Status_Time"><value>${pair.isTimePassed ? "PASS" : "FAIL"}</value></Data>
      <Data name="Avg_Y"><value>${pair.avgE}</value></Data>
      <Data name="Avg_X"><value>${pair.avgN}</value></Data>
      <Data name="Height"><value>${hVal}</value></Data>
      <Data name="Height_Type"><value>${this.isTg20Applied ? "ORTHOMETRIC_TG20" : "ELLIPSOIDAL"}</value></Data>
      <Data name="Lat"><value>${lat.toFixed(8)}</value></Data>
      <Data name="Lon"><value>${lon.toFixed(8)}</value></Data>
    </ExtendedData>
    <Point>
    ...
    <name>${pt.pn}</name>
    <ExtendedData>
      <Data name="Point_Name"><value>${pt.pn}</value></Data>
      <Data name="Type"><value>SINGLE_OBS</value></Data>
      <Data name="Y"><value>${pt.e.toFixed(3)}</value></Data>
      <Data name="X"><value>${pt.n.toFixed(3)}</value></Data>
      <Data name="Height"><value>${hVal}</value></Data>
      <Data name="Height_Type"><value>${this.isTg20Applied ? "ORTHOMETRIC_TG20" : "ELLIPSOIDAL"}</value></Data>
      <Data name="Lat"><value>${lat.toFixed(8)}</value></Data>
      <Data name="Lon"><value>${lon.toFixed(8)}</value></Data>
  ...
  exportCadastreCsv() {
    let csv = this.isTg20Applied
      ? "Point_Name,Date_1,Time_1,Date_2,Time_2,Diff_Time_Min,Diff_Time_Hours,dY_cm,dX_cm,dH_cm,dS_cm,Status_Dist,Status_Time,Avg_Y,Avg_X,Avg_H,TG20_N,Ortho_H,Type\n"
      : "Point_Name,Date_1,Time_1,Date_2,Time_2,Diff_Time_Min,Diff_Time_Hours,dY_cm,dX_cm,dH_cm,dS_cm,Status_Dist,Status_Time,Avg_Y,Avg_X,Avg_H,Type\n";

    for (let pair of this.matchedPairs) {
      if (this.isTg20Applied) {
        csv += `${pair.pointName},${pair.p1.dt},${pair.p1.tm},${pair.p2.dt},${pair.p2.tm},${pair.timeDiffMin},${pair.timeDiffHours},${pair.dy},${pair.dx},${pair.dh},${pair.ds2d},${pair.isDistPassed ? "PASS" : "FAIL"},${pair.isTimePassed ? "PASS" : "FAIL"},${pair.avgE},${pair.avgN},${pair.avgH},${pair.tg20N || ""},${pair.avgOrthoH || pair.avgH},PAIR_AVG\n`;
      } else {
        csv += `${pair.pointName},${pair.p1.dt},${pair.p1.tm},${pair.p2.dt},${pair.p2.tm},${pair.timeDiffMin},${pair.timeDiffHours},${pair.dy},${pair.dx},${pair.dh},${pair.ds2d},${pair.isDistPassed ? "PASS" : "FAIL"},${pair.isTimePassed ? "PASS" : "FAIL"},${pair.avgE},${pair.avgN},${pair.avgH},PAIR_AVG\n`;
      }
    }

    for (let pt of this.unmatchedPoints) {
      if (this.isTg20Applied) {
        csv += `${pt.pn},${pt.dt || ""},${pt.tm || ""},--,--,--,--,--,--,--,--,--,--,${pt.e.toFixed(3)},${pt.n.toFixed(3)},${pt.h.toFixed(3)},${pt.tg20N || ""},${pt.orthoH !== undefined ? pt.orthoH.toFixed(3) : pt.h.toFixed(3)},SINGLE_OBS\n`;
      } else {
        csv += `${pt.pn},${pt.dt || ""},${pt.tm || ""},--,--,--,--,--,--,--,--,--,--,${pt.e.toFixed(3)},${pt.n.toFixed(3)},${pt.h.toFixed(3)},SINGLE_OBS\n`;
      }
    }
  ...
  /**
   * Resmi Kadastro Çift Okuma Karne verilerini saf nesne (DTO) olarak döner
   */
  getCadastreReportData(title = "CADASTRE_KARNE") {
    return {
      title,
      date: new Date().toISOString().slice(0, 10),
      centralMeridian: this.centralMeridian,
      isTg20Applied: this.isTg20Applied,
      pairs: this.matchedPairs || [],
      unmatched: this.unmatchedPoints || []
    };
  }

  /**
   * Yazdırılabilir Resmi Kadastro Çift Okuma Çetelesi HTML Çıktısı Üretir
   * Şablon doldurma işini GnssReportTemplates yöneticisine devreder.
   */
  generatePrintableCadastreReport(title = "CADASTRE_KARNE") {
    const karneData = this.getCadastreReportData(title);
    if (typeof window !== "undefined" && window.GnssReportTemplates?.renderCadastreKarne) {
      return window.GnssReportTemplates.renderCadastreKarne(karneData);
    }
    return "";
  }
