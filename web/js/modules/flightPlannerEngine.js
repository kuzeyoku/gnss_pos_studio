...
class FlightPlannerEngine {
  ...
  async parsePolygonFile(file) {
    ...
    if (ext === "kmz") {
      ...
      if (!kmlFile) {
        throw new Error("ERR_KMZ_NO_DOC_KML");
      }
    ...
    if (!coordinates || coordinates.length < 3) {
      throw new Error("ERR_INVALID_POLYGON_MIN_POINTS");
    }
  ...
  _parseKml(kmlString) {
    ...
    if (!longestCoordText) {
      throw new Error("ERR_KML_NO_COORDINATES");
    }
  ...
  _parseGeoJson(geoJsonString) {
    ...
    if (!polyCoords) {
      throw new Error("ERR_GEOJSON_NO_POLYGON");
    }
  ...
  addCustomRoad(coords, name = "Road", roadType = "track") {
  ...
  async parseRoadFile(file) {
    ...
    if (ext === "kmz") {
      const zip = await JSZip.loadAsync(file);
      const kmlFile = Object.values(zip.files).find(f => f.name.toLowerCase().endsWith(".kml"));
      if (!kmlFile) throw new Error("ERR_KMZ_NO_DOC_KML");
      textContent = await kmlFile.async("text");
    } else {
      textContent = await file.text();
    }
    ...
    if (ext === "geojson" || ext === "json") {
        ...
        if (feat.geometry && (feat.geometry.type === "LineString" || feat.geometry.type === "MultiLineString")) {
          ...
          lines.forEach((lineCoords, lIdx) => {
            importedRoads.push({
              ...
              name: feat.properties?.name || feat.properties?.YOL_ADI || `Road #${idx + 1}`,
              type: "track",
      ...
      placemarks.forEach((pm, idx) => {
        ...
        const roadName = nameNode ? nameNode.textContent.trim() : `Road #${idx + 1}`;
    ...
    if (importedRoads.length === 0) {
      throw new Error("ERR_NO_ROAD_GEOMETRY");
    }
  ...
  async generateSmartGCPs({
    ...
    if (!polygon || polygon.length < 4) {
      throw new Error("ERR_NO_SURVEY_BOUNDARY");
    }
    ...
    resolvedPoints.forEach((pt, idx) => {
      ...
      let statusCode = "OPEN_TERRAIN";
      let statusDesc = "";
      if (pt.roadDistM !== null) {
        statusCode = pt.roadDistM <= 15 ? "ROADSIDE" : "NEAR_ROAD";
        ...
        statusDesc = pt.roadDistM <= 15 ? `Roadside (~${pt.roadDistM}m)${roadName}${roadType}` : `Near Road (${pt.roadDistM}m)${roadName}${roadType}`;
      } else {
        statusCode = pt.isInterior ? "OPEN_INTERIOR" : "OPEN_BOUNDARY";
        statusDesc = pt.isInterior ? "Open Field (Interior)" : "Open Field (Boundary)";
      }
  ...
  recalculatePointRoadDistance(point, snapMaxRadiusM = 500) {
    ...
    if (this.roadWays && this.roadWays.length > 0) {
      ...
      if (nearest && nearest.distanceM <= snapMaxRadiusM) {
        ...
        point.statusCode = point.roadDistM <= 15 ? "ROADSIDE" : "NEAR_ROAD";
        point.status = point.roadDistM <= 15 ? `Roadside (~${point.roadDistM}m)${name}${type}` : `Near Road (${point.roadDistM}m)${name}${type}`;
      } else {
        point.nearestRoad = null;
        point.roadDistM = null;
        point.statusCode = "OPEN_TERRAIN";
        point.status = "Open Field (>500m)";
      }
    }
  }
  ...
  _formatRoadType(type) {
    if (typeof window !== "undefined" && typeof window.t === "function") {
      const translated = window.t(`flight.roadTypes.${type}`);
      if (translated && !translated.startsWith("flight.roadTypes.")) return translated;
    }
    return type || "track";
  }
  ...
  calculateSolarTrajectory(lat, lon, dateObj = new Date()) {
    ...
    for (let minuteOfDay = 360; minuteOfDay <= 1140; minuteOfDay += 15) {
      ...
      let shadowCategory = "NIGHT";

      if (elevationDeg > 0) {
        shadowMultiplier = 1.0 / Math.tan(elevationRad);
        if (elevationDeg >= 45.0) shadowCategory = "EXCELLENT";
        else if (elevationDeg >= 35.0) shadowCategory = "GOOD";
        else if (elevationDeg >= 25.0) shadowCategory = "MODERATE";
        else shadowCategory = "POOR";
      }
    ...
    this.solarData = {
      ...
      optimalWindow: optimalStart && optimalEnd ? `${optimalStart} - ${optimalEnd}` : null,
  ...
  async fetchLiveWeather(lat, lon, targetDate = null) {
    ...
    try {
      ...
      if (!resp.ok) throw new Error("ERR_WEATHER_API_FAIL");

      const data = await resp.json();
      if (!data.hourly || !data.hourly.time) throw new Error("ERR_WEATHER_DATA_EMPTY");
      ...
      for (let i = 0; i < hourly.time.length; i++) {
        ...
        const windDir = hourly.wind_direction_10m[i] !== undefined ? hourly.wind_direction_10m[i] : 0;
        ...
        const cloudCover = hourly.cloud_cover[i] !== undefined ? hourly.cloud_cover[i] : 0;

        if (hourNum >= 7 && hourNum <= 18) {
      ...
      this.weatherData = {
        ...
        optimalFlightHeading: `${optimalHeading1}° / ${optimalHeading2}°`,
      ...
      this.weatherData = {
        ...
        lat: lat,
        lon: lon,
        dataSource: "Internal Simulation",
        isLive: false,
        maxWindMs: 5.8,
        maxWindKmh: 21.0,
        maxGustMs: 7.8,
        ...
        avgCloudPct: 15,
        avgWindDir: 285,
        optimalFlightHeading: "15° / 195°",
        overallSafety: "safe",
        overallText: "WEATHER_SAFE",
        overallBadge: "SAFE",
        hours: fallbackHours
      };
      return this.weatherData;
  ...
  calculateFlightParameters({
    ...
    if (!camera) {
      camera = {
        id: "m3e_built_in",
        name: "DJI Mavic 3 Enterprise Built-in (4/3 20MP)",
        sensorW: 17.3,
        sensorH: 13.0,
        focalMm: 12.29,
        imageW: 5280,
...
<Document>
  <name>Flight Plan - ${grid.drone?.model || "UAV"}</name>
  <description>Altitude: ${grid.flightAltitudeM}m, Lines: ${grid.totalLinesCount}, Photos: ${grid.totalPhotosCount}, Distance: ${grid.totalDistanceKm}km</description>
  ...
  <Folder>
    <name>Flight Lines</name>
`;

    grid.lines.forEach((line, idx) => {
      kml += `
    <Placemark>
      <name>Line #${idx + 1} (${line.lengthM}m)</name>
...
`;
  ...
  exportGcpCsv() {
    if (!this.gcpPoints || this.gcpPoints.length === 0) return "";
    let csv = "Point_Name,Point_Type,WGS84_Lat,WGS84_Lon,ITRF96_Y,ITRF96_X,DOM,Nearest_Road_Distance_m,Status\n";
    for (let pt of this.gcpPoints) {
      const distStr = pt.roadDistM !== null ? pt.roadDistM : "Open_Terrain";
...
<Document>
  <name>Ground Control Points (GCP)</name>
`;
  ...
  <Placemark>
    ...
    <description>Point: ${pt.name} | Type: ${pt.type} | ITRF_Y: ${(pt.itrfY || 0).toFixed(3)} | ITRF_X: ${(pt.itrfX || 0).toFixed(3)} | DOM: ${pt.dom || 30}</description>
    <ExtendedData>
      <Data name="Point_Name"><value>${pt.name}</value></Data>
      <Data name="Point_Type"><value>${pt.type}</value></Data>
      <Data name="ITRF_Y"><value>${(pt.itrfY || 0).toFixed(3)}</value></Data>
      <Data name="ITRF_X"><value>${(pt.itrfX || 0).toFixed(3)}</value></Data>
      <Data name="DOM"><value>${pt.dom || 30}</value></Data>
      <Data name="Status"><value>${pt.status || ""}</value></Data>
    </ExtendedData>
    <Point>
