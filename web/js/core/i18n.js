...
(function (window, document) {
  ...
  class StudioI18n {
    ...
    t(key, params = {}, fallback = null) {
      ...
      // Otomatik Hata Kodu Çözümleme (ERR_* -> errors.ERR_*)
      if (typeof key === 'string' && key.startsWith('ERR_')) {
        const fromErr = this.t(`errors.${key}`, params, null);
        if (fromErr && fromErr !== `errors.${key}`) return fromErr;
      }
    ...
    translateDOM(root = document) {
      ...
      root.querySelectorAll('[data-i18n-tooltip]').forEach(el => {
        ...
        const translated = this.t(key);
        if (translated) el.setAttribute('data-tooltip', translated);
      });

      // Optgroup / input labels
      root.querySelectorAll('[data-i18n-label]').forEach(el => {
        const key = el.getAttribute('data-i18n-label');
        const translated = this.t(key);
        if (translated) el.setAttribute('label', translated);
      });
    }
  }
  ...
  window.resolveError = (err, fallback = '') => {
    if (!err) return fallback;
    const msg = typeof err === 'string' ? err : (err.message || String(err));
    return i18nInstance.t(msg, {}, msg || fallback);
  };
