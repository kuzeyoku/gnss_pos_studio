# Consolidation script for components.css
# Adheres to the unified design system:
# 1. Base .btn + modifiers (.btn-primary, .btn-secondary, etc.)
# 2. Base .form-input / .form-select / .form-textarea + modifiers
# 3. Base .badge / .chip + color & interactive modifiers
# 4. Base .card / .glass-panel + modifiers
# 5. Base .table + sticky header
# 6. Safe .modal-overlay with display: none default!

css_content = '''/* ==========================================================================
   HARITA TOOLS GNSS WEB STUDIO — COMPONENTS (CONSOLIDATED DESIGN SYSTEM)
   ========================================================================== */

/* ==========================================================================
   01. CARDS, PANELS & CONTAINERS
   ========================================================================== */
.card,
.glass-panel,
.config-panel,
...
.flight-time-slider {
  ...
  height: 6px;
  border-radius: 3px;
  background: var(--surface-card-a90);
  outline: none;
...
.card.app-map-card {
  ...
  overflow: hidden;
  position: relative;
  background: rgba(var(--surface-card-rgb), 0.55);
  padding: 0;
}
...
.switch-glass input:checked + .switch-slider {
  ...
  border-color: var(--cyan-400);
  box-shadow: 0 0 10px var(--cyan-a40);
}

.switch-glass input:checked + .switch-slider:before {
...
/* ==========================================================================
   11. TAB SPECIALIZED LAYOUTS (CLEAN GRIDS & VIEWPORTS ONLY)
   ========================================================================== */

/* Home Tab Hero */
.card.home-hero-banner {
  position: relative;
  overflow: hidden;
  background: linear-gradient(135deg, var(--surface-card-a90) 0%, rgba(var(--surface-side-rgb), 0.95) 100%);
...
.card.home-hero-banner::before {
  ...
  width: 320px;
  height: 320px;
  background: radial-gradient(circle, var(--cyan-a15) 0%, transparent 70%);
  pointer-events: none;
}
...
.trust-icon { font-size: 22px; color: var(--cyan-400); flex-shrink: 0; }
.trust-item strong { display: block; font-size: 12.5px; color: #ffffff; margin-bottom: 2px; }
.trust-item span   { font-size: 11px; color: var(--text-dim); line-height: 1.4; display: block; }
...
print(f"components.css rewritten successfully: {len(css_content.splitlines())} lines.")
